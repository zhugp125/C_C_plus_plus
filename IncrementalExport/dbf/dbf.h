...
EXTERN_C DBF_HANDLE  dbf_create_attach (void* stream, const struct zlib_filefunc_def_s*, const DBF_FIELD_INFO* array, dbf_uint array_count, enum dbf_charconv, void* memo);
...
EXTERN_C BOOL        dbf_putrecord_del (DBF_HANDLE, dbf_uint index, const dbf_uint offset);
EXTERN_C BOOL        dbf_addrecord     (DBF_HANDLE);

#if 1
EXTERN_C BOOL        dbf_appendrecord   (DBF_HANDLE, const char *data);
EXTERN_C int         dbf_refresh        (DBF_HANDLE);
#endif

EXTERN_C BOOL        dbf_insertrecord  (DBF_HANDLE, dbf_uint index);
EXTERN_C BOOL        dbf_isrecorddeleted(DBF_HANDLE);
EXTERN_C BOOL        dbf_deleterecord  (DBF_HANDLE, BOOL do_delete, const dbf_uint offset);
