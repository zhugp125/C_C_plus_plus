...
void dbf_write_header(DBF_HANDLE handle)
{
   ...
   // write eof character
   //ZWRITE(handle->api, handle->stream, "\0x1a", 1);
}
...
{
    return dbf_putrecord_del(handle, record, 0);
}


EXTERN_C BOOL dbf_putrecord_del(DBF_HANDLE handle, dbf_uint record, const dbf_uint offset)
{
    BOOL ok = (record < handle->recordcount);

    if (ok)
    {
        dbf_setposition(handle, record);
        ZSEEK(handle->api, handle->stream, handle->headerlength + (record - 0) * handle->recordlength + offset, ZLIB_FILEFUNC_SEEK_SET);
...
BOOL dbf_deleterecord(DBF_HANDLE handle, BOOL do_delete, const dbf_uint offset)
{
   ...
   if (ok)
   {
      ...
      handle->modified = TRUE;
      ok = dbf_putrecord_del(handle, dbf_getposition(handle), offset);
      if (ok)
      {
          //加上这句话，就会物理删除这条记录
          //不加这句话会产生一个bug，dbf记录超过324条，删除就会产生问题：末尾会多出几条未知的记录
          --(handle->recordcount);
      }
   }
   else
...
#if 1
BOOL dbf_appendrecord(DBF_HANDLE handle, const char *data)
{
    // get the length of each field, including the decimal points
    dbf_uint nField = dbf_getfieldcount(handle);
    std::vector<size_t> fieldLengths;
    for(dbf_uint i=0; i<nField; ++i)
    {
        const DBF_FIELD *field = dbf_getfieldptr(handle, i);
        fieldLengths.push_back(field->m_Length);
    }
    ...
    for(dbf_uint i=0; i<nField; ++i)
    {
        fields.push_back(tmpData.substr(0, fieldLengths[i]));
        tmpData = tmpData.substr(fieldLengths[i], tmpData.size()-fieldLengths[i]);
    }

    // insert the record at end of file
    if(!dbf_addrecord(handle))
    {
        return false;
    }
    
    // copy the record
    dbf_setposition(handle, handle->recordcount-1);
    for(dbf_uint i=0; i<nField; ++i)
    {
        if((!dbf_putfield(handle, dbf_getfieldptr(handle, i), fields[i].c_str())) || (!dbf_putrecord(handle, handle->recordcount-1)))
        {
           return false;
        }
    }
...
int dbf_refresh(DBF_HANDLE handle)
{
    return 0;
}
#endif
